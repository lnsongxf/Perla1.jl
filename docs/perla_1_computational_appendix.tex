...
\end{align}
where $b(i')$ denotes the cohort firm $i'$ belongs to. Here we drop the $a$ since the $f$ vector provides the sufficient information to calculate demand and prices.

Note that, conditional on $n_b > 0$, the probability of a $b$-cohort firm being included in the $n_b$ awared firms out of total $N$ firms is $n_b /N$ (Polya urn). By (3) in paper, \cref{eq:full-total-demand-multiple-cohorts} can be written in terms of cross-cohort awareness counts as
\begin{align}
y_{ib}(p_i, p_{-i}, f) 
